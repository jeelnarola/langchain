...
#         while iteration < max_iterations:
#             iteration += 1
#             print('\033[92m=====iteration=====\033[0m',iteration)
#             did_end_loop = await self.make_api_requests()
#             ended, result = did_end_loop
#             if ended:
#                 print('\033[92m=====result=====\033[0m', result)
...
#             # If no completion tag found, continue the loop to get proper completion
#             if not tool_blocks and "<attempt_completion>" not in assistant_reply:
#                 print("⚠️ No completion tag found, continuing loop...")
#                 self.add_to_history("assistant", assistant_reply)
#                 return False, None
...
class ToolAgent:
    ...
    async def start_task(
        ...
        memory_text = retrieve_memory_db(self.db, k=3)  # last 3 saved memory items # last 3 saved memory items
    ...
    async def make_api_requests(self) -> Tuple[bool, Optional[str]]:
        try:
            ...
            assistant_reply = (
                ...
                or "<thinking>Processing...</thinking>"
            )
            print("\033[92m=====assistant_reply=====\033[0m", assistant_reply)

            # -------------------- Execute <use_mcp_tool> blocks --------------------
            tool_blocks = re.findall(
                r"<use_mcp_tool>.*?</use_mcp_tool>", assistant_reply, re.DOTALL
            )
            ...
            # -------------------- Check for <attempt_completion> --------------------
            if (
                "<attempt_completion>" in assistant_reply
                and "</attempt_completion>" in assistant_reply
            ):
                start = assistant_reply.find("<attempt_completion>") + len(
                    "<attempt_completion>"
                )
                end = assistant_reply.find("</attempt_completion>")
                content = assistant_reply[start:end].strip()
